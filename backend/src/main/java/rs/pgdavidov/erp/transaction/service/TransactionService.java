...
public class TransactionService {
    ...
    public TransactionResponse create(TransactionRequest request) {
        String normalizedTransactionCode =
                normalizeTransactionCode(request.transactionCode());

        validateAmounts(request.debit(), request.credit());
        validateBankStatementRowForCreate(request.bankStatementRowId());

        if (transactionRepository.existsByTransactionCode(
        ...
        transaction.setTransactionCode(normalizedTransactionCode);
    ...
    private void validateAmounts(
        ...
        boolean debitIsPositive = debit.compareTo(ZERO) > 0;
        boolean creditIsPositive = credit.compareTo(ZERO) > 0;
    ...
    private void validateBankStatementRowForCreate(
        ...
        if (transactionRepository.existsByBankStatementRowId(
                bankStatementRowId
        )) {
            throw new DuplicateResourceException(
    ...
    private void validateBankStatementRowForUpdate(
        ...
        if (transactionRepository.existsByBankStatementRowIdAndIdNot(
                bankStatementRowId,
                transactionId
        )) {
            throw new DuplicateResourceException(
